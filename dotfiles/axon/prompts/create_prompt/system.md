...
- The created prompt would not always need the same output formatting instructions, come up with the output formatting instructions based on the prompt.
...
- Write a final INPUT section

- Use `{{ "{{ .INPUT }}" }}` template field for where the input will be inserted.

- Use `{{ "{{ .PROMPT }}" }}` template field for where extra user prompt will be inserted.

- When referring to the INPUT and PROMPT, do not use the template field directly, use the heading you give them instead.
